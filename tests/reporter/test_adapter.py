...
expected_html_report: Report = Report(
    ...
    [
        ReportModel(
            ...
            [ReportRule("a rule name", "", ReportStatus.PASSED)],
            ReportSummary(1, 0, 0),
        ),
        ReportModel(
            "model2",
            ...
            [
                ReportRule(
                    "a rule name",
                    "this rule has failed because something has failed",
...
def test_should_convert_to_html_output():
    rule: Rule = Rule(
        "a rule name", "this rule has failed because something has failed", None
    )
